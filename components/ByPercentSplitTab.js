...
const PercentSplitCard = ({ contact, currencyCode }) => {
    ...
    const [amount, setAmount] = useState(split_list[index]?.split_by_percent?.toString() || '');
    ...
    const handleAmountChange = (text) => {
        ...
        if (numericRegex.test(text)) {
            dispatch(setSplitAmount({ contact, amount: parseFloat(text), type: 'BY_PERCENTAGE' }));
            setAmtErr(false);
        }
        else {
            dispatch(setSplitAmount({ contact, amount: null, type: 'BY_PERCENTAGE' }));
            setAmtErr(true);
        }
    };
        ...
        <View style={styles.container}>
            ...
            <FlatList
                ...
                renderItem={({ item: { contact }, index }) => {
                    return <PercentSplitCard contact={contact} currencyCode="%" />;
                }}
                ListFooterComponent={<View style={styles.bottomBar}>
                    <Text style={[styles.bottomText, { color: 'black' }]}>{totalAmount} out of 100%</Text>
                    <Text style={[styles.bottomText, { color: amountLeft < 0 ? theme.error : amountLeft === 0 ? theme.success : 'gray' }]}>{amountLeft}% left</Text>
                </View>}
            />
        </View>
    );
...
export default ByPercentSplitTab;
